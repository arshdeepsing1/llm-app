...
it('opens Databricks-only settings and saves only the editable Databricks fields', async () => {
  ...
  expect(dialog.getByText('Current estimated input budget: 120,832 tokens.', { exact: false })).toBeTruthy()
  fireEvent.change(dialog.getByLabelText('Databricks model endpoint'), { target: { value: 'databricks-claude-opus-4-8' } })
  fireEvent.change(budget, { target: { value: '131000' } })
  fireEvent.change(output, { target: { value: '121000' } })
  expect(dialog.getByText(/leaves too little room for a working conversation/)).toBeTruthy()
  expect(dialog.getByText(/reserves max_tokens against a 20,000 output-tokens-per-minute limit/)).toBeTruthy()
  fireEvent.change(output, { target: { value: '20000' } })
  expect(dialog.getByText(/reserves the full standard Claude Opus 4.8 output-per-minute quota/)).toBeTruthy()
  expect(dialog.getByText(/8,192 default leaves room for multi-step work/)).toBeTruthy()
  fireEvent.change(dialog.getByLabelText('Databricks model endpoint'), { target: { value: 'test-model' } })
  fireEvent.change(budget, { target: { value: '131072' } })
