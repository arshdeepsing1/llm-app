...
type Props = { session: Session | null; onClose: () => void; onSelectSession: (id: string) => void; onError: (message: string) => void }
type Checkpoint = { id: string; path: string; created: number; status: string }
...
function AgentToolsScope(props: Props) {
  ...
  return <dialog className="agent-tools-dialog" ref={dialog} aria-label="Agent tools" onCancel={props.onClose} onClick={e => { if (e.target === dialog.current) props.onClose() }}>
    <div className="agent-tools-content">
      ...
      <nav className="agent-tools-tabs" aria-label="Agent tools sections">{['Recovery', 'Worktrees', 'Tasks', 'Extensions'].map(name => <button key={name} aria-pressed={tab === name} onClick={() => setTab(name)}>{name}</button>)}</nav>
      {tab === 'Recovery' ? <Recovery {...props} /> : tab === 'Worktrees' ? <Worktrees {...props} /> : tab === 'Tasks' ? <Tasks {...props} /> : <ExtensionsPanel {...props} />}
    </div>
  </dialog>
}
...
function Recovery({ session, onError }: Props) {
  ...
  return <section className="agent-tools-section" aria-label="Recovery">
    ...
    <p>Preview a saved checkpoint before restoring a file.</p>
    {error ? <p className="form-error" role="alert">{error}</p> : null}
    {notice ? <p role="status">{notice}</p> : null}
    <div className="agent-tools-list">{items.map(item => <div className="agent-tools-row" key={item.id}><div><strong>{item.path}</strong><small>{item.status}</small></div>
      <button className="outline" disabled={busy} aria-label={`Preview checkpoint ${item.path}`} onClick={() => { setNotice(''); setPreview(null); void run(() => api<Preview>(`/checkpoints/${encodeURIComponent(item.id)}/preview${query}`), setPreview) }}>Preview</button></div>)}</div>
    {!busy && !items.length ? <p className="muted">No saved file checkpoints in this workspace scope.</p> : null}
    {preview ? <div className="agent-tools-preview"><h4>{preview.path}</h4><pre>{preview.diff || 'No content difference.'}</pre>
      ...
      }, items => { setItems(items); setNotice(`Restored ${preview.path}.`); setPreview(null) })}>Restore checkpoint</button>
    </div> : null}
  </section>
}
...
function SessionTasks({ session, onError, onSelectSession, onClose }: Props & { session: Session }) {
  ...
  return <section className="agent-tools-section" aria-label="Tasks">
    ...
    <div className="agent-tools-list">{children.map(child => <div className="agent-tools-row" key={child.id}><div><strong>{child.title}</strong><small>{child.status === 'awaiting_approval' ? 'Waiting for approval' : child.status}</small></div><button className="outline" aria-label={`Open subagent ${child.title}`} onClick={() => { onSelectSession(child.id); onClose() }}>Open</button></div>)}</div>
...
function ExtensionsPanel({ session, onError }: Props) {
  ...
  return <section className="agent-tools-section" aria-label="Extensions">
    ...
    <p>Configure stdio commands or HTTP server URLs and before_tool / after_tool hooks. Saving authorizes enabled servers to start.</p>
    ...
    <details className="agent-tools-example"><summary>Configuration example</summary><pre>{JSON.stringify({ servers: [{ id: 'example', name: 'Example MCP', transport: 'stdio', command: 'your-mcp-server', args: [], enabled: false }], hooks: [{ id: 'example-hook', event: 'before_tool', command: 'your-hook-command', timeout_seconds: 10, enabled: false }] }, null, 2)}</pre></details>
    <div className="agent-tools-actions"><button className="primary" disabled={!loaded || busy || !configuration.trim()} onClick={() => { setNotice(''); setTools(null); void run(() => api<Extensions>('/extensions', 'PUT', JSON.parse(configuration)), config => { setConfiguration(JSON.stringify(config, null, 2)); setNotice('Configuration saved.') }) }}>Save configuration</button>
      <button className="outline" disabled={busy} onClick={() => { setNotice(''); void run(() => api<{ tools: string[] }>('/extensions/test', 'POST'), result => setTools(result.tools)) }}>Test saved configuration</button></div>
